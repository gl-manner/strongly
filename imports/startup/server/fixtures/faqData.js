...
// Startup logic to seed FAQ data
export async function seedFaqData() {
  console.log('Checking FAQ data...');

  const existingFaqs = await FaqCollection.find().countAsync();
  console.log(`Found ${existingFaqs} existing FAQ items`);
  ...
  if (existingFaqs === 0) {
    ...
    console.log(`Inserted ${faqFixtures.length} FAQ items`);
    ...
    const missingFaqs = [];
    for (const faq of faqFixtures) {
      const existing = await FaqCollection.findOneAsync(faq._id);
      if (!existing) {
        missingFaqs.push(faq);
      }
    }

    if (missingFaqs.length > 0) {
      console.log(`Adding ${missingFaqs.length} missing FAQ items...`);
  ...
  console.log('FAQ data check complete');
}
